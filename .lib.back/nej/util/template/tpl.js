...
NEJ.define([
    ...
    _p._$parseTemplate = (function() {
        ...
        var _doParseSrc = function(_textarea, _options) {
            ...
            // bugfix textarea data-src not corrent for safari in mac
            if (!_src) _src = _textarea.getAttribute('data-src');
            if (!_src) return;
        ...
        return function(_element, _options) {
            ...
            if (!!_element) {
                var _list = _element.tagName == 'TEXTAREA' ? [_element] : _u._$object2array(
